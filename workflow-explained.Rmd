...
Data upload to EDI will happen in [make_metadata_xml.R](https://github.com/FlowWest/edi-feather-snorkel/blob/edi-workflow-documentation/data-raw/files_for_updating_EDI/make_metadata_xml.R) script. Modifications needed on this script are:
  ...
  - Uncomment this code line and run. (note: running this code will automatically upload the EDI package. Packages can not be overwritten, so if any changes are needed, both (1) new edi number on line 40 and (2) update_edi_package on line 75 will have to be updated, and script has to be ran again)
