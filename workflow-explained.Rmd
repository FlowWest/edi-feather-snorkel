...
## Adding Data
1. Add the new data to the Microsoft Access Database (`Snorkel_Revised.mdb`), located in the `data-raw/db-files` folder.
2. After entering the data, save the updated `.mdb` file.
3. Save the Access database as an `.accdb` file with `*_PC*` appended to the file name. This new file should be named `Snorkel_Revised_PC.accdb`.

## Processing Data
Data processing and uploading are handled by two scripts located in the `files_for_uploading_EDI` folder.
